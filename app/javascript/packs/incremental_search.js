...
if (!!document.getElementsByClassName("black")[0]) {
  if (document.getElementsByClassName("black")[0].value === '質問') {
    data['question_or_article'] = '質問';
  } else {
    data['question_or_article'] = '記事';
  }
}
// ルートページ以外の個人用検索フォーム(/articles/id or /questions/id)
if (data['question_or_article'] === '') {
  // URLのidの部分を抜き出す
...
// チェックボックスを押す度に、送信データを表すdataのプロパティ(の値)が変わる仕組み
// accuracy_arrayの設定
for (let i = 0; i < accuracy_array.length; i++) {
  accuracy_array[i].addEventListener('click',function(){
    if (accuracy_array[i].checked) {
      data['accuracy'].push(accuracy_array[i].value);
    }else{
      data['accuracy'] = data['accuracy'].filter((eachvalue, index, array) => {
...
for (let i = 0; i < difficulty_array.length; i++) {
  difficulty_array[i].addEventListener('click', function(){
    if (difficulty_array[i].checked) {
      data['difficulty'].push(difficulty_array[i].value);
    } else {
      data['difficulty'] = data['difficulty'].filter((eachvalue, index, array) => {
...
for (let i = 0; i < solve_array.length; i++) {
  solve_array[i].addEventListener('click', function(){
    if (solve_array[i].checked) {
      data['solve'].push(solve_array[i].value);
    } else {
      data['solve'] = data['solve'].filter((eachvalue, index, array) => {
...
for (let i = 0; i < term_array.length; i++) {
  term_array[i].addEventListener('change', function(){
    data['term'][i] = term_array[i].value;
  });
}
...
search_word.addEventListener('keyup', () => {
  ...
  const options = {
    ...
    body: JSON.stringify(data) //JavaScriptのオブジェクトや値(送信データ)を JSON 文字列に変換
  };
...
// ---------- 取得した検索結果を画面に表示させる ----------
    ...
    let match_data_max_ten = data.length;
    ...
    search_result_count.insertAdjacentText('afterbegin',`${data.length}件`);
    ...
    for (let i = 0; i < match_data_max_ten; i++) {
      ...
      if (data['question_or_article'] === '質問') {
        link_area.setAttribute("href", `/questions/${data[i]['id']}/browsing`);
      } else {
        link_area.setAttribute("href", `/articles/${data[i]['id']}/browsing`);
      }
      link_area.insertAdjacentText('afterbegin', data[i]['title']);
