...
RSpec.describe "<system>UserLogins", type: :system do
  ...
  it "login succeeds when user submits valid information" do
    ...
    aggregate_failures do
      ...
      expect(page).to have_link '仮ログアウト', href: logout_path
      expect(page).to have_link '仮アカウント更新', href: edit_user_path(@user)
    end

    click_on '仮ログアウト'
    aggregate_failures do
      expect(current_path).to eq root_path
      expect(page).to have_no_link '仮ログアウト'
      expect(page).to have_no_link '仮アカウント更新'
    end
  end
end
